...
var loadingBtn = "<svg version=\"1.1\" id=\"loader-1\" xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\" x=\"0px\" y=\"0px\" width=\"20px\" height=\"20px\" viewBox=\"0 0 50 50\" style=\"enable-background:new 0 0 50 50;\" xml:space=\"preserve\">\n    <path fill=\"#fff\" d=\"M25.251,6.461c-10.318,0-18.683,8.365-18.683,18.683h4.068c0-8.071,6.543-14.615,14.615-14.615V6.461z\">\n      <animateTransform attributeType=\"xml\" attributeName=\"transform\" type=\"rotate\" from=\"0 25 25\" to=\"360 25 25\" dur=\"0.6s\" repeatCount=\"indefinite\"></animateTransform>\n      </path>\n    </svg>";
$(document).ready(function () {
  $("body").off().on("click", ".menu-mobile-icon button", function () {
  ...
  function formatDateWithDayOfWeek(date) {
    ...
    return "".concat(dayOfWeek, ", ").concat(day, "/").concat(month, "/").concat(year);
  }
  // Initialize with the values from the image

  var isMobile = window.matchMedia("(max-width: 768px)").matches;
  // Departure date picker
  var departureDatePicker = flatpickr("#departure-date", {
    dateFormat: "d/m/Y",
    minDate: "today",
    ...
    // hiển thị 2 tháng
    locale: "vn",
    // dùng thứ tiếng Việt: T2, T3,...
    disableMobile: true,
    // luôn dùng desktop UI
    onChange: function onChange(selectedDates) {
      if (selectedDates[0]) {
        ...
        if (returnDatePicker.selectedDates[0]) {
          $("#return-date").val(formatDateWithDayOfWeek(returnDatePicker.selectedDates[0]));
        } else {
  ...
  var departureInputValue = $("#departure-date").data("date");
  var departureDate = departureInputValue ? new Date(departureInputValue) : new Date();
  ...
  var returnInputValue = $("#return-date").data("date");
  if (returnInputValue) {
    var returnDate = new Date(returnInputValue);
  ...
  $("body").on("click", "#swap-locations", function () {
    ...
    var departureDisplayValue = $("#departure").data("name");
    var destinationDisplayValue = $("#destination").data("name");
  ...
  // search page filter
  function fetchFilteredResults(startMinutes, endMinutes, minPrice, maxPrice) {
    var type = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : "replace";
    var filterUrl = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : "";
    var fillter = {
      timeRange: formatTimeForDisplay(startMinutes) + " - " + formatTimeForDisplay(endMinutes),
    ...
    $.ajax({
      ...
      error: function error(_error) {
        console.error("Error fetching results:", _error);
        loadingBox("close");
      }
    });
  }
  ...
  $("#price-slider").slider({
    ...
    // 2,000,000 VND
    values: [0, 2000000],
    // Default values (0 - 2,000,000 VND)
    step: 5000,
    ...
    stop: function stop(event, ui) {
      ...
      window.priceSliderTimeout = setTimeout(function () {
        ...
        var timeValues = $("#time-slider").slider("values");
        // Call AJAX function with current values
        fetchFilteredResults(timeValues[0], timeValues[1], ui.values[0], ui.values[1]);
      }, 400);
    }
  });
  ...
  $("#time-slider").slider({
    ...
    stop: function stop(event, ui) {
      ...
      window.sliderTimeout = setTimeout(function () {
        ...
        var priceValues = $("#price-slider").slider("values");
        fetchFilteredResults(ui.values[0], ui.values[1], priceValues[0], priceValues[1]);
      }, 400);
    }
  });
  ...
  $("body").on("click", ".sidebar .button-toggle", function () {
    var button = $(this);
    button.closest(".sidebar-section").find(".section-content").slideToggle(300, function () {
  ...
  $(".sidebar-section .search-input").on("input", function () {
    ...
    $(this).parent().find(".company-item").each(function () {
      var companyName = $(this).find("label").text().toLowerCase();
  ...
  $("body").on("click", ".load-more", function () {
    ...
    var priceValues = $("#price-slider").slider("values");
    $(this).remove();
    fetchFilteredResults(timeValues[0], timeValues[1], priceValues[0], priceValues[1], "append", filterUrl);
  });
  ...
  $('.company-item input[type="checkbox"]').on("change", function () {
    ...
    // check is parrent
    var isParent = $(this).closest(".company-item").hasClass("parent");
    // checked all child if is parent
    if (isParent) {
      $(this).closest(".filter-item").find(".list-child input[type='checkbox']").prop("checked", isChecked);
    }
    ...
    window.checkboxTimeout = setTimeout(function () {
      ...
      var priceValues = $("#price-slider").slider("values");
      // Call AJAX function with current values
      fetchFilteredResults(timeValues[0], timeValues[1], priceValues[0], priceValues[1]);
    }, 400);
  });

  // Initialize proper arrow directions based on initial state
  // If sections are initially visible, arrows should point up
  $(".sidebar-section").find(".section-content").each(function () {
    if ($(this).is(":visible")) {
      $(this).closest(".sidebar-section").find(".button-toggle svg").addClass("rotate-180");
    }
  });

  // slider
  $(".product-detail__content-item.product-images").each(function () {
    var mySwiper = $(this).find(".mySwiper")[0];
    var mySwiper2 = $(this).find(".mySwiper2")[0];
  ...
  $("body").on("click", ".product-detail__tab-item", function () {
    ...
    $(this).closest(".product-detail").find(".product-detail__tab-item").removeClass("active");
    $(this).addClass("active");
    $(this).closest(".product-detail").find(".product-detail__content-item").removeClass("active");
    $(this).closest(".product-detail").find(".product-detail__content-item." + tab).addClass("active");
  });
  $("body").on("click", ".open-information", function () {
    $(this).toggleClass("active");
    $(this).closest(".border-box").find(".product-detail").toggleClass("active");
  });
  $("body").on("click", ".close-information", function () {
    $(this).closest(".border-box").find(".open-information").removeClass("active");
    $(this).closest(".border-box").find(".product-detail").removeClass("active");
  });
  ...
  $("body").on("click", ".open-popup", function () {
    ...
    var productPriceFormat = $(this).data("product-price-format");
    ...
    var checkIsReturnBooking = $("#popup-booking").find("button[type='submit']").data("isShowReturn");
    ...
    var locations = $(this).closest(".border-box").find(".info-location").data("locations");
    if (locations && locations.length > 0) {
      var locationPickupHtml = "";
      var locationPickup = locations.filter(function (item) {
        return item.type === 'pickup';
      });
      var locationPickupFirst = locations.filter(function (item) {
        return item.type === 'pickup';
      }).slice(0, 1)[0];
      $.each(locationPickup, function (index, value) {
        locationPickupHtml += "<option value=\"".concat(value.id, "\" data-transit=\"").concat(value === null || value === void 0 ? void 0 : value.transit, "\">").concat(value.time, " - ").concat(value.name, "</option>");
      });
      $("#popup-booking").find("select[name='pickup']").html(locationPickupHtml);
      $("#popup-booking").find("select[name='pickup']").val((locationPickupFirst === null || locationPickupFirst === void 0 ? void 0 : locationPickupFirst.id) || 0).change();
      var locationDropOffHtml = "";
      var locationDropOff = locations.filter(function (item) {
        return item.type === 'dropoff';
      });
      var locationDropOffLast = locations.filter(function (item) {
        return item.type === 'dropoff';
      }).slice(-1)[0];
      $.each(locationDropOff, function (index, value) {
        locationDropOffHtml += "<option value=\"".concat(value.id, "\" data-transit=\"").concat(value === null || value === void 0 ? void 0 : value.transit, "\">").concat(value.time, " - ").concat(value.name, "</option>\n        </div>");
      });
      $("#popup-booking").find("select[name='dropoff']").html(locationDropOffHtml);
      $("#popup-booking").find("select[name='dropoff']").val((locationDropOffLast === null || locationDropOffLast === void 0 ? void 0 : locationDropOffLast.id) || 0).change();
    }
  });
  $("body").on("change", ".select-location", function () {
    var selectedOption = $(this).find("option:selected");
    var transitTime = selectedOption.data("transit");
    if (transitTime) {
  ...
  $("body").on("click", ".btn-qty", function () {
    var qty = $(this).parent().find("input.quantity");
    ...
    qty.val(value);
    var price = $("#popup-booking").find("input.product-price").val();
    $("#popup-booking").find(".total-price").text(formatCurrency(value * price));
  });
  $("body").on("submit", ".form-booking", function (e) {
    e.preventDefault();
    var button = $(this).find("button[type='submit']");
    var checkIsReturnBooking = button.data("isShowReturn");
    var pickUpId = $(this).find("select[name='pickup']").val();
    var dropOffId = $(this).find("select[name='dropoff']").val();
    var pickUpAddress = $(this).find('textarea[name="pickup_detail"]').val();
    var dropOffAddress = $(this).find('textarea[name="dropoff_detail"]').val();
    var formData = {
        ...
        if (response.error) {
          ...
          if (returnDatePicker.selectedDates[0] && !checkIsReturnBooking) {
            ...
            window.checkboxTimeout = setTimeout(function () {
              ...
              // Call AJAX function with current values
              fetchFilteredResults(timeValues[0], timeValues[1], priceValues[0], priceValues[1], 'replace', filterUrl);
            }, 400);
          }
        }
        button.find("#loader-1").remove();
