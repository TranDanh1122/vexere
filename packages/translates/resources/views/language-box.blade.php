...
>
    {!! languageFlag($currentLanguage->lang_flag, $currentLanguage->lang_name, 24) !!}
...
@if (count($languages) > 1)
    <div>
        <h4>{{ trans('Translate::language.translations') }}</h4>
        <div id="list-others-language">
            @foreach ($languages as $language)
                ...
                @if (array_key_exists($language->lang_code, $related))
                    ...
                    >
                        {!! languageFlag($language->lang_flag, $language->lang_name) !!}
    ...
    <div data-change-language-route="{{ route('admin.languages.change.item.language') }}"></div>

    <x-Core::modal.action
        id="confirm-change-language-modal"
        type="warning"
        :title="trans('Translate::language.confirm-change-language')"
        :description="BaseHelper::clean(trans('Translate::language.confirm-change-language-message'))"
        :submit-button-attrs="['id' => 'confirm-change-language-button']"
        :submit-button-label="trans('Translate::language.confirm-change-language-btn')"
    />
@endif
