	...
	<tr data-table="{{$table_name??''}}" data-id="{!! $value->id !!}">
		...
		@foreach ($data['table_generate'] as $generate)
			@switch($generate['type'])
		        	...
		        	@if (isset($has_locale) && $has_locale == true && !empty($languages ?? []) && count($languages) > 1)
		        		<td class="lang">
		        			@foreach ($languages as $key => $lang)
		        				...
		        				@endphp
		        				@if (isset($lang_data))
		        					<span><a href="{{ route('admin.'.$table_name.'.edit', $lang_data->lang_table_id) }}?lang_locale={{ $key }}"><i class="fas fa-check text-pink"></i></a></span>
		        				@else
		        ...
		        @break
		        @case('action_show')
			        @include('Table::components.action',['type' => 'action_show'])
		        @break
