        ...
        (function ($) {
            ...
            ZdCascader.prototype._wrapClick = function (event) {
                event.stopPropagation();
                this.$el.focus();
            ...
            ZdCascader.prototype._loadChildren = function ($parentNode) {
                ...
                $.each(data.children, (i, m) => {
                    var $li = $(`<li class="${this.CLASS.menuNode}" id="zd-item-${m.value}"></li>`);
            ...
            ZdCascader.prototype.selectedItem = function() {
                if(this.$selectedItemId.length) {
                    $(`input#${this.options.inputValueName}`).trigger('click');
                    let fullText = ''
                    this.$selectedItemId.map((item, index) => {
                        $(`#zd-item-${item}`).trigger('click');
                        if(index != this.$selectedItemId.length - 1) {
                            fullText += $(`#zd-item-${item} .zd-cascader-node__label`).text().trim() + ' / ';
                        } else {
                            $(`#value_${this.options.inputValueName}`).val(item)
                            fullText += $(`#zd-item-${item} .zd-cascader-node__label`).text().trim()
                        }
                    })
                    $(`input#${this.options.inputValueName}`).val(fullText)
                    $('.zd-cascader-wrap').removeClass('is-focus')
                }
            }
