...
namespace DreamTeam\Base\Traits;
...
trait LoadAndPublishDataTrait
{
    ...
    public function setNamespace(string $namespace): self
    {
        ...
        $this->app['config']->set(['app.plugin_namespaces.' . File::basename($this->getPath()) => $namespace]);
    ...
    protected function getPath(string $path = null): string
    {
        ...
        $modulePath = str_replace(['/src/Providers', '\src\Providers'], '', File::dirname($reflection->getFilename()));

        if (! Str::contains($modulePath, base_path('plugins'))) {
            $modulePath = base_path('plugins/' . $this->getDashedNamespace());
        }
