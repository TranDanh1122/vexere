    ...
    function format_price(
        ...
        if ($currency) {
            if (! $currency instanceof Currency) {
                $currency = app(CurrencyServiceInterface::class)->findOne(['id' => $currency], false);
            }
        ...
        if ($useSymbol && $currency->is_prefix_symbol) {
            $space = (int)get_currency_setting('add_space_between_price_and_currency', 0) == 1 ? ' ' : null;
...
if (! function_exists('human_price_text')) {
    function human_price_text(float|null $price, Currency|null|string $currency, string $priceUnit = ''): string
    {
        ...
        if (get_currency_setting('display_big_money_in_million_billion', false)) {
            if ($price >= 1000000 && $price < 1000000000) {
                $price = round($price / 1000000, 2) + 0;
                $priceUnit = __('Core::currency.million') . ' ' . $priceUnit;
                $numberAfterDot = strlen(substr(strrchr((string)$price, '.'), 1));
            } elseif ($price >= 1000000000) {
                $price = round($price / 1000000000, 2) + 0;
                $priceUnit = __('Core::currency.billion') . ' ' . $priceUnit;
                $numberAfterDot = strlen(substr(strrchr((string)$price, '.'), 1));
            }
        }
        ...
        $decimalSeparator = get_currency_setting('decimal_separator', '.');

        if ($decimalSeparator == 'space') {
            $decimalSeparator = ' ';
        }

        $thousandSeparator = get_currency_setting('thousands_separator', ',');

        if ($thousandSeparator == 'space') {
            $thousandSeparator = ' ';
        }
        if($price == 0 && CurrencyFacade::currencyTextDefault()) {
            return CurrencyFacade::currencyTextDefault();
        }
        $price = number_format(
            (float)$price,
            (int)$numberAfterDot,
...
if (! function_exists('get_application_currency')) {
    function get_application_currency(): ?Currency
    {
        ...
        if (\Auth::guard('admin')->check() || ! $currency) {
...
if(!function_exists('get_currency_setting')) {
    ...
    {
        $currency = getOption('currency', null, false);
        return $currency[$key] ?? $default;
    }
}
