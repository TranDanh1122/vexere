...
class MediaManagement {
    ...
    handleMediaList() {
        ...
        _self.$body
            ...
            .on("click", ".js-media-list-title", (event) => {
                ...
                if (shift_key) {
                    let firstItem = Helpers.arrayFirst(
                        Helpers.getSelectedItems()
                    );
                    if (firstItem) {
        ...
        _self.$body
            ...
            .on("click", ".js-rv-media-change-view-type button", (event) => {
                ...
                if (typeof RV_MEDIA_CONFIG.pagination != "undefined") {
                    if (
                        typeof RV_MEDIA_CONFIG.pagination.paged != "undefined"
                    ) {
                        RV_MEDIA_CONFIG.pagination.paged = 1;
                    }
                    if (MediaConfig.request_params.view_type == 'details') {
                        RV_MEDIA_CONFIG.pagination.has_more = true
                    }
                }
    ...
    changeFilter() {
        ...
        _self.$body
            ...
            .on("click", ".js-rv-media-change-filter", (event) => {
                event.preventDefault();
        ...
        _self.$body
            ...
            .on(
                ...
                (event) => {
                    ...
                    let ele_options =
                        typeof window.rvMedia.$el !== "undefined"
            ...
            .on("submit", ".form-add-folder", (event) => {
                event.preventDefault();
                const $input = $(event.currentTarget).find(
                    'input[name="name"]'
                );
    ...
    handleModals() {
        ...
        _self.$body.on("show.bs.modal", "#modal-properties", (event) => {
            if (Helpers.getSelectedItems().length === 1) {
                ...
                $modal
                    .find(`input[name="color"][value="${selected.color}"]`)
                    .prop("checked", true);
            }
        });
                ...
                ActionsService.processAction(
                    {
                        ...
                        selected: Helpers.getSelectedItems().map((item) =>
                            item.id.toString()
                        ),
                    },
                    () => {
                ...
                ActionsService.processAction(
                    ...
                    (res) => {
                        if (!res.error) {
                            ...
                            $("#modal_alt_text_items .mb-3").each(
                                (index, el) => {
                                    let $current = $(el);
                                    if (
                                        Helpers.inArray(
        ...
        /*Delete files*/
        _self.$body
            .off("submit", "form.form-delete-items")
            .on("submit", "form.form-delete-items", (event) => {
                event.preventDefault();
                let items = [];
                let $form = $(event.currentTarget);
    ...
    bindIntegrateModalEvents() {
        ...
        $mainModal
            .off(
                "dblclick doubletap",
                '.js-media-list-title[data-context="file"]'
            )
            .on(
                "dblclick doubletap",
                '.js-media-list-title[data-context="file"]',
        ...
        $(".rv-media-main .rv-media-items").bind(
            ...
            (e) => {
                if (
                    ...
                    if (loadMore) {
                        if (
                            typeof RV_MEDIA_CONFIG.pagination != "undefined" &&
