...
export class UploadService {
    ...
    setupDropZone() {
        ...
        _self.dropZone = new Dropzone(document.querySelector('.rv-media-items'), {
            ...
            sending: function (file, xhr, formData) {
                ...
                formData.append('allow_webp', Helpers.getRequestParams().allow_webp || 'no')
                formData.append('allow_thumb', Helpers.getRequestParams().allow_thumb || 'no')
                formData.append('module_name', Helpers.getRequestParams().module_name || '')
    ...
    changeProgressStatus(file) {
        ...
        if (file.status === 'error') {
            ...
            if (response.data.refresh_folder && response.data.refresh_folder == true && response.data.folder_id && Helpers.getRequestParams().view_type != 'details') {
                _self.FolderService.changeFolder(response.data.folder_id);
            }
