...
export class ActionsService {
    ...
    static handlePreview() {
        ...
        if (Helpers.size(selected) > 0) {
            DreamTeamCore.lightbox(selected)
    ...
    static renderCropImage() {
        ...
        if (RV_MEDIA_CONFIG.is_using_cloud) {
            $.ajax({
                url: '/admin/media/getContent',
                method: 'POST',
                data: {
                    url: item.full_url,
                ...
                beforeSend: function() {
                    DreamTeamCore.showLoading(document.getElementById('rv_crop_media_loading'))
                },
                success: function (data) {
                    const img = new Image();
                    const url = window.URL || window.webkitURL;
                    const src = url.createObjectURL(data);
                    DreamTeamCore.hideLoading(document.getElementById('rv_crop_media_loading'))
                    img.onload = function() {
                        ...
                        const el = html.replace(/__src__/gi, dataURL);
                        modal.append(el);
    
                        imageElement = modal.find('img')[0];
        ...
        function initializeCropper(image) {
            ...
            form.find('#aspectRatio').off('click').on('click', function () {
                cropper.destroy();
                if ($(this).is(':checked')) {
