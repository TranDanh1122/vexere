...
namespace DreamTeam\Media\Storage\BunnyCDN;

use DreamTeam\Media\Storage\BunnyCDN\Exceptions\BunnyCDNException;
use DreamTeam\Media\Storage\BunnyCDN\Exceptions\DirectoryNotEmptyException;
use DreamTeam\Media\Storage\BunnyCDN\Exceptions\NotFoundException;
...
class BunnyCDNClient
{
    ...
    /**
     * Calculate the total storage size of the specified path.
     * 
     * @param string $path
     * @return int
     *
     * @throws BunnyCDNException
     */
    public function calculateStorageSize(string $path): int
    {
        try {
            $listing = $this->list($path);
            $totalSize = 0;
            ...
            foreach ($listing as $item) {
                ...
                if ($item['IsDirectory']) {
                    $totalSize += $this->calculateStorageSize($path . '/' . $item['ObjectName']);
                }
            }
